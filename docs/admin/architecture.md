...
flowchart TD
  ...
  src -->|"Bootstrap Course Org<br/>--propagate-secret"| orgsec["each org's DSL_BOT_TOKEN<br/>ORG secret<br/>visibility = selected → .github (+ welcome, classroom-config)"]
  src -->|"Bootstrap, same run"| infrasec["REPO secret on each<br/>PRIVATE infra repo<br/>classroom-config"]
  src -->|"Refresh actions"| reposec["REPO secret on each<br/>private content repo<br/>materials-* (not assignment-*)"]
  orgsec --> pub["public .github / welcome<br/>workflows authenticate"]
  infrasec --> disp["classroom-config's<br/>dispatch workflows authenticate"]
