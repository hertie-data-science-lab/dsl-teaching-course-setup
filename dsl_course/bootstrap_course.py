...
- DSL_BOT_TOKEN secret (required for all workflows), plus the central GRAPH_*/SMTP_*
  mail secrets when they are configured centrally
...
# The mail-transport secrets the enrolment-code and grade-distribution buttons need, in the
# exact names dsl_course.mailer reads (graph_config_from_env / smtp_config_from_env): Graph
# is preferred, SMTP is the fallback, and mailer picks whichever set is complete. They are
# held ONCE as secrets on the CENTRAL repo (one shared no-reply mailbox for every course)
# and copied onto each org from the central Bootstrap workflow's env, exactly like
# DSL_BOT_TOKEN - otherwise every course org would need the same nine secrets pasted in by
# hand before it could email anyone.
MAIL_SECRETS = [
    "GRAPH_TENANT_ID",
    "GRAPH_CLIENT_ID",
    "GRAPH_CLIENT_SECRET",
...
def propagate_mail_secrets(org: str, repos: list[str] | None = None) -> int:
    ...
    infra repos, so the mail credentials reach the public `.github` repo the send buttons
    run in, and no student-facing/content repo. `repos` is computed once here and reused
    for all nine rather than re-probing the org per secret.
    ...
    if not present:
        ...
        return 0
    repos = repos or _infra_repos(org)
    count = sum(set_org_secret(org, name, value, repos) for name, value in present)
    log_ok(f"{count} mail secret(s) propagated")
    return count
...
def main() -> int:
    ...
    parser.add_argument(
        "--propagate-secret",
        action="store_true",
        help="Set DSL_BOT_TOKEN on this org to the DSL_BOT_TOKEN/GH_TOKEN env value, plus "
        "any GRAPH_*/SMTP_* mail secrets present in env (lets the central bootstrap "
        "auto-provision them - no manual per-org step).",
    )
    parser.add_argument(
        "--admins",
        default="",
    ...
    # its settings. Runs on cohort bootstraps too: the send buttons only ever run in a
    # COURSE org, but keeping one code path is simpler than a course-only special case,
    # and the scoping is identical to the bot token already propagated there. A no-op
    # when nothing is configured centrally.
    if args.set_secret or args.propagate_secret:
        propagate_mail_secrets(args.org)
